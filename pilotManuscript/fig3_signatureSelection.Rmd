...
selectDataMatAndPlot<-function(drugName,
                               method,
                               data,genelist,
                               doEnrich=FALSE,
                               doGraph=FALSE){
  ...
  if(doGraph){
    ##build graph
    ures<-res%>%
      dplyr::select(Gene,Datatype)%>%
      distinct()
    
    gene.vals<-rep(1,nrow(subset(ures,Datatype=='binaryMutations')))
    names(gene.vals)<-subset(ures,Datatype=='binaryMutations')$Gene

    prot.vals<-rep(1,nrow(subset(ures,Datatype=='proteinLevels')))
    names(prot.vals)<-subset(ures,Datatype=='proteinLevels')$Gene
    
    rna.vals<-rep(1,nrow(subset(ures,Datatype=='mRNALevels')))
    names(rna.vals)<-subset(ures,Datatype=='mRNALevels')$Gene
    
  #  print(rna.vals)
    phos.vals<-rep(1,nrow(subset(ures,Datatype=='Phosphosite')))
    names(phos.vals)<-subset(ures,Datatype=='Phosphosite')$Gene
    
    fname=paste0(drugName,'_',method,'_',data,'network')
    netres<-computePhosphoNetwork(phos.vals,prot.vals,gene.vals,rna.vals,fname=fname)
  }

  ##rename duplicated genes/transcripts
...
```{r plot graphs}
  ...
  graph.results<-all.preds%>% 
    ...
    mutate(doGraph=TRUE)%>%
   mutate(doEnrich=FALSE)%>%
    subset(compound%in%c('Trametinib (GSK1120212)','Quizartinib (AC220)'))%>%
    subset(meanCor>0.5)%>%
    dplyr::select(var='compound',method,data='Molecular',genelist='genes',doEnrich,doGraph)%>%
    rowwise()%>%
    mutate(enrich=paste(selectDataMatAndPlot(var,method,data,genelist,doEnrich,doGraph)$Description,collapse=','))
