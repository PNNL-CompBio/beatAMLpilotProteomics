...
res<-lapply(folds,function(testpats){
 ...
 print("Phospho preds first")
  substrate.dat<-pat.phos%>%
    ...
    phospho.reg.results<-drugMolRegressionEval(auc.dat.fam,
                                               tr.dat,
                                               'Phosphosite',
                                               dplyr::rename(auc.dat.fam,Sample='AML sample'),
    ...
    phospho.enet.results<-drugMolRegressionEval(auc.dat.fam,
                                                tr.dat,
                                                'Phosphosite',
                                                dplyr::rename(auc.dat.fam,Sample='AML sample'),
                                                dplyr::rename(te.dat,Sample='AML sample'),
                                                category='family', enet.alpha = seq(0.1,0.9,0.1))%>%
        do.call(rbind,.)
    phospho.enet.results <- phospho.enet.results[, -1]
      
    print("Working on Phospho Logistic")
    phospho.lr.results<-drugMolLogRegEval(auc.dat.fam,tr.dat,'Phosphosite',
                                          dplyr::rename(auc.dat.fam,Sample='AML sample'),
                                          dplyr::rename(te.dat,Sample='AML sample'),
                                          category='family')%>%
      do.call(rbind,.)
  ...
  reg.preds<-purrr::map_df(list(mRNA='mRNALevels',
                         ...
                         ~ drugMolRegressionEval(auc.dat.fam,tr.dat,.x,
                                                 dplyr::rename(auc.dat.fam,Sample='AML sample'),
                                                 dplyr::rename(te.dat,Sample='AML sample'),category='family'))
  ...
  enet.preds<-purrr::map_df(list(mRNA='mRNALevels',
                             ...
                             #   gene='geneMutations',
                                 binGene='binaryMutations'),
                            ~ drugMolRegressionEval(auc.dat.fam,tr.dat,.x, enet.alpha = seq(0.1,0.9,0.1),
                                                    dplyr::rename(auc.dat.fam,Sample='AML sample'),
                                                    dplyr::rename(te.dat,Sample='AML sample'),category='family'))  
  ...
  log.reg.preds<-purrr::map_df(list(protein='proteinLevels',
                                ...
                                #   gene='geneMutations',
                                    binGene='binaryMutations'),
                               ~ drugMolLogRegEval(auc.dat.fam,tr.dat,.x,
                                                   dplyr::rename(auc.dat.fam,Sample='AML sample'),
                                                   dplyr::rename(te.dat,Sample='AML sample'),category='family'))
  ...
  return(rbind(full.results,enet.results,lr.results))
})
...
ggsave('5foldCVFamilyperformance.pdf',p3)
