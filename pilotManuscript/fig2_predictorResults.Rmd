...
```{r build predictors, echo=FALSE, warning=FALSE}
  ...
  l.sum.tab<-logr.preds%>%subset(numFeatures>0)%>%
    group_by(Molecular)%>%summarize(NumberModels=n_distinct(var),Error=mean(MSE),SamplesUsed=mean(numSamples,na.rm=T))%>%mutate(Type='Logistic')
  
  sum.tab<-reg.preds%>%subset(numFeatures>0)%>%
    group_by(Molecular)%>%
    summarize(NumberModels=n_distinct(var),Error=mean(MSE), SamplesUsed=mean(numSamples,na.rm=T))%>%mutate(Type='LASSO')%>%
    rbind(l.sum.tab)
  knitr::kable(sum.tab)
